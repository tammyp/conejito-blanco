...
class Parvulo extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre, rut, contrasena, talla, peso, email, fecha_nacimiento', 'required'),
			array('talla, peso', 'numerical'),
			array('nombre', 'length', 'max'=>100),
			array('rut, contrasena', 'length', 'max'=>20),
			array('email', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('nombre, rut, contrasena, talla, peso, email, fecha_nacimiento', 'safe', 'on'=>'search'),
		);
	}
